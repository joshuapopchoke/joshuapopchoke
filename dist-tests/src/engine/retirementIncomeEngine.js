...
exports.buildRetirementIncomeSnapshot = buildRetirementIncomeSnapshot;
function computeLongValue(holdings, tickers) {
    return Object.values(holdings).reduce((total, holding) => total + (tickers[holding.ticker]?.price ?? 0) * holding.shares, 0);
}
function computeShortValue(holdings, tickers) {
    return Object.values(holdings).reduce((total, holding) => total + (tickers[holding.ticker]?.price ?? 0) * holding.shares, 0);
}
...
function annualPortfolioIncome(client, tickers) {
...
function parseSpendingRuleRate(spendingRule) {
    if (!spendingRule) {
        return null;
    }
...
function describeWithdrawalRate(withdrawalRate) {
...
function describeRunway(cashRunwayMonths) {
...
function buildRetirementIncomeSnapshot(client, tickers) {
    ...
    const pressureScore = Math.max(0, Math.min(100, (coverageRatio >= 1 ? 0 : (1 - coverageRatio) * 55) +
