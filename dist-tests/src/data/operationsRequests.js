...
function buildRequest(client, cycleNumber, requestId, title, summary, prompt, options) {
    return {
        clientId: client.id,
...
function getOperationsRequest(client, difficulty, cycleNumber) {
    const simplerPrompt = difficulty === "learner" || difficulty === "trainee";
    switch (client.id) {
        case "retiree":
            return buildRequest(client, cycleNumber, "rmd", "RMD processing request", `${client.name} asked for help handling an IRA distribution so the retirement plan stays on track without creating unnecessary confusion.`, simplerPrompt
        ...
        case "young_pro":
            return buildRequest(client, cycleNumber, "trusted-contact", "Trusted contact and beneficiary review", `${client.name} wants to clean up the account file after a job change and asked whether anything important is missing from the registration records.`, simplerPrompt
        ...
        case "family":
            return buildRequest(client, cycleNumber, "beneficiary-529", "529 and beneficiary maintenance request", `${client.name} wants the family education and beneficiary records cleaned up so the planning picture is easier to understand.`, simplerPrompt
        ...
        case "entrepreneur":
            return buildRequest(client, cycleNumber, "registration-change", "Registration change request", `${client.name} wants help deciding whether ownership and beneficiary records should be updated after recent business planning changes.`, simplerPrompt
        ...
        case "institutional":
            return buildRequest(client, cycleNumber, "authorized-contact", "Authorized contact maintenance request", `${client.name} needs the operating file updated so the right people can approve future instructions and reserve transfers.`, simplerPrompt
