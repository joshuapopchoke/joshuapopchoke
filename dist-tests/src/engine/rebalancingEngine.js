...
exports.buildClientAllocationTargets = buildClientAllocationTargets;
exports.analyzePortfolioDrift = analyzePortfolioDrift;
exports.buildRebalancePlan = buildRebalancePlan;
exports.applyClientRebalance = applyClientRebalance;
const CORE_BUCKETS = ["stocks", "funds", "fixedIncome", "bonds", "forex", "commodities", "futures"];
function normalizeTargets(targets) {
    const filtered = targets.filter((target) => CORE_BUCKETS.includes(target));
    return filtered.length > 0 ? filtered : ["funds"];
}
function defaultWeightMap(client) {
    const map = client.riskProfile === "Conservative"
...
function buildClientAllocationTargets(client) {
    const allowed = normalizeTargets(client.mandateTargets);
    const defaults = defaultWeightMap(client);
    const activeEntries = allowed.map((bucket) => [bucket, defaults[bucket] ?? (1 / allowed.length)]);
    const total = activeEntries.reduce((sum, [, weight]) => sum + weight, 0) || 1;
    return Object.fromEntries(activeEntries.map(([bucket, weight]) => [bucket, weight / total]));
}
function analyzePortfolioDrift(client, tickers) {
    const targets = buildClientAllocationTargets(client);
    const totalValue = client.cash +
        Object.values(client.holdings).reduce((sum, holding) => sum + (tickers[holding.ticker]?.price ?? 0) * holding.shares, 0);
    const currentWeights = Object.values(client.holdings).reduce((accumulator, holding) => {
    ...
    const rows = Object.entries(targets).map(([bucket, targetWeight]) => ({
        bucket,
        targetWeight,
    ...
    const summary = driftScore >= 35 ? "Portfolio drift is high enough that the mandate is starting to look off-target." :
...
function preferredTickerForBucket(bucket, client, tickers) {
...
function buildRebalancePlan(client, tickers) {
...
function applyClientRebalance(client, tickers) {
    const plan = buildRebalancePlan(client, tickers);
    let nextCash = client.cash;
    const nextHoldings = { ...client.holdings };
