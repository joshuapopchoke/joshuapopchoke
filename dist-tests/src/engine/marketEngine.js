...
exports.createMarketEngine = createMarketEngine;
const rateEngine_1 = require("./rateEngine");
const STOCKS = [
...
function inferBeta(ticker) {
    if (ticker.category === "stocks") {
        ...
        if (ticker.sector === "Healthcare")
            return 0.92;
        if (ticker.sector === "Energy")
            return 1.18;
        if (ticker.sector === "Consumer")
            return 0.86;
        if (ticker.sector === "Industrials")
            return 1.04;
        if (ticker.sector === "Real Estate")
            return 0.9;
        if (ticker.sector === "Utilities")
            return 0.62;
        return 1;
    }
    if (ticker.category === "funds") {
        if (ticker.sector === "Index Funds")
            return 1;
        if (ticker.sector === "Small Cap Funds")
            return 1.18;
        if (ticker.sector === "Mid Cap Funds")
            return 1.07;
        if (ticker.sector === "Large Cap Funds")
            return 0.96;
        if (ticker.sector === "Mutual Funds")
            return 0.88;
        if (ticker.sector === "Active Funds")
            return 1.04;
        if (ticker.sector === "Sector Funds")
            return 1.16;
        if (ticker.sector === "Hybrid Funds")
            return 0.68;
        if (ticker.sector === "International Funds")
            return 0.94;
        if (ticker.sector === "Target Date Funds")
            return 0.71;
        if (ticker.sector === "Hedge Funds")
            return 0.82;
        return 0.9;
    }
    if (ticker.category === "fixedIncome")
        return 0.18;
    if (ticker.category === "bonds")
        return 0.32;
    if (ticker.category === "forex")
        return 0.55;
    if (ticker.category === "commodities")
        return 0.74;
    if (ticker.category === "futures")
        return 1.15;
    return 1;
}
function isLastTradingDaysOfYear(date) {
    if (date.getMonth() !== 11) {
        return false;
    }
    return date.getDate() >= 24;
}
function isFirstTradingDaysOfYear(date) {
    return date.getMonth() === 0 && date.getDate() <= 4;
}
function isTurnOfMonth(date) {
    return date.getDate() <= 3 || date.getDate() >= 28;
}
function inHalloweenWindow(date) {
    return date.getMonth() >= 10 || date.getMonth() <= 3;
}
function isSeptember(date) {
    return date.getMonth() === 8;
}
function inSadWindow(date) {
    return date.getMonth() >= 8 || date.getMonth() <= 1;
}
class MarketEngine {
    constructor() {
        this.tickers = Object.fromEntries(ALL_TICKERS.map((ticker) => [
            ticker.symbol,
            {
                ...ticker,
        ...
        this.tickers = (0, rateEngine_1.refreshCallableBondTerms)(this.tickers, 1);
    ...
    simulateTick(volatilityMultiplier = 1, eventChance = 0.06, marketDate = new Date()) {
    ...
    refreshStructuralTerms(cycleNumber) {
        this.tickers = (0, rateEngine_1.refreshCallableBondTerms)(this.tickers, cycleNumber);
        return this.getState();
    }
    seededRand(symbol, offset) {
    ...
    getVolatility(symbol) {
        if (["NVDA", "META", "AMZN", "GOOGL", "TSLA", "NFLX", "AMD"].includes(symbol))
            return 0.02;
        if (["AAPL", "MSFT", "GS", "CRM", "ORCL", "ADBE", "SCHW", "LLY"].includes(symbol))
            return 0.016;
        if (["BA", "COP", "ABBV", "DIS", "RTX"].includes(symbol))
            return 0.018;
        if (["PFE", "KO", "PG", "NEE"].includes(symbol))
            return 0.008;
        if (["UNH", "COST", "CAT", "V", "HD", "MCD", "HON", "UPS", "PLD"].includes(symbol))
            return 0.012;
        if (["JPM", "BAC", "MS", "XOM", "CVX", "MRK", "CSCO"].includes(symbol))
            return 0.014;
        if (["BRK", "WMT", "AMT", "GE"].includes(symbol))
            return 0.01;
        if (["SPY", "VTI", "VOO", "IVV", "VFIAX", "FXAIX", "SCHX"].includes(symbol))
            return 0.006;
        if (["QQQ", "IWM", "IJH"].includes(symbol))
            return 0.009;
        if (["VWELX", "VWINX", "AOR", "VTTSX", "SWYNX"].includes(symbol))
            return 0.004;
        if (["VXUS", "VEA", "VWO"].includes(symbol))
            return 0.007;
        if (["AGTHX", "FCNTX", "PRNHX"].includes(symbol))
            return 0.01;
        if (["HFLS", "HFGM"].includes(symbol))
            return 0.015;
        if (["IBM30", "JPM31", "MUNI34"].includes(symbol))
            return 0.004;
        if (["T40", "VZ38", "ET35"].includes(symbol))
            return 0.006;
        if (["F32", "CCL31"].includes(symbol))
            return 0.009;
        if (symbol.includes("/") && (symbol.includes("JPY") || symbol.includes("BRL")))
            return 0.004;
        if (symbol.includes("/") && ["MXN", "CNY", "NOK", "SEK"].some((currency) => symbol.includes(currency)))
            return 0.006;
        if (symbol.includes("/"))
            return 0.003;
        if (["GLD", "GC"].includes(symbol))
            return 0.009;
        if (["SLV", "COPPER"].includes(symbol))
            return 0.013;
        if (["OIL", "CL", "NAT"].includes(symbol))
            return 0.022;
        if (["CORN", "SOY", "WHT"].includes(symbol))
            return 0.016;
        if (["ES", "NQ", "YM", "RTY"].includes(symbol))
            return 0.009;
        if (["BIL", "SHY"].includes(symbol))
            return 0.0016;
        if (["IEF", "TIP", "MUB"].includes(symbol))
            return 0.003;
        if (["TLT", "LQD"].includes(symbol))
            return 0.0045;
        if (["HYG"].includes(symbol))
            return 0.006;
        if (["ZB", "ZN"].includes(symbol))
            return 0.005;
        return 0.013;
    }
