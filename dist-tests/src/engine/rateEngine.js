...
exports.buildInterestRateSnapshot = buildInterestRateSnapshot;
exports.refreshCallableBondTerms = refreshCallableBondTerms;
function seededValue(seedText, offset) {
    let total = 0;
...
function syntheticDayIndex(cycleNumber) {
    return Math.max(0, cycleNumber - 1);
}
function syntheticMonthIndex(cycleNumber) {
    return Math.floor(syntheticDayIndex(cycleNumber) / 20);
}
function syntheticCalendarLabel(cycleNumber) {
...
function buildInterestRateSnapshot(cycleNumber, period) {
...
function refreshCallableBondTerms(tickers, cycleNumber) {
    const monthIndex = syntheticMonthIndex(cycleNumber);
    return Object.fromEntries(Object.entries(tickers).map(([symbol, ticker]) => {
